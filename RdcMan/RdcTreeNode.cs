...
namespace RdcMan
{
	public abstract class RdcTreeNode : TreeNode, ILogonCredentials
	{
		...
		public virtual EncryptionSettings EncryptionSettings
		{
			get
			{
				return FileGroup.EncryptionSettings;
			}
			protected set
			{
		...
		protected void ReadXml(Dictionary<string, Helpers.ReadXmlDelegate> nodeActions, XmlNode xmlNode, ICollection<string> errors)
		{
			...
			{
				nodeActions.TryGetValue(childNode.Name, out Helpers.ReadXmlDelegate value);
				try
				{
					if (value != null)
					{
				...
				{
					errors.Add("{0} 在 '{1}' 中读取Xml节点异常: {2}".InvariantFormat(childNode.GetFullPath(), base.Text, ex.Message));
				}
			}
		}
