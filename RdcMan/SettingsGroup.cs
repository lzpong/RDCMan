		...
		internal void ReadXml(XmlNode xmlNode, RdcTreeNode node, ICollection<string> errors) {
			...
			if (result != InheritanceMode.Disabled) {
				try {
					...
					if (!Enum.TryParse<InheritanceMode>(value, out result)) {
						errors.Add("{1} 中的意外继承模式“{0}”".InvariantFormat(value, xmlNode.GetFullPath()));
						result = InheritanceMode.None;
					}
				}
				catch {
					errors.Add("{0} 中未指定继承模式".InvariantFormat(xmlNode.GetFullPath()));
					result = InheritanceMode.None;
				}
					...
					foreach (XmlNode childNode in xmlNode.ChildNodes) {
						try {
							...
							try {
								...
								errors.Add("处理 Xml 节点 {0} 时出错".InvariantFormat(childNode.GetFullPath()));
							}
						}
						catch {
							errors.Add("意外的 Xml 节点 {0}".InvariantFormat(childNode.GetFullPath()));
						}
					}
					break;
				}
				default:
					errors.Add("{1} 中的意外继承模式“{0}”".InvariantFormat(result.ToString(), xmlNode.GetFullPath()));
					break;
			}
		}
