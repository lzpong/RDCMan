...
namespace RdcMan
{
	...
	internal class RecentlyUsedGroup : BuiltInVirtualGroup<RecentlyUsedServerRef>, IServerRefFactory
	{
		private class RecentlyUsedSettings : GroupSettings
		{
			...
			private new const string TabName = "属性";
			...
			public IntSetting MaxNumberOfServers
			{
				get;
				private set;
			}
		...
		{
			base.Properties = new RecentlyUsedSettings();
		...
		{
			using (TabbedSettingsDialog tabbedSettingsDialog = new TabbedSettingsDialog("最近使用的组设置", "OK", parentForm))
			{
