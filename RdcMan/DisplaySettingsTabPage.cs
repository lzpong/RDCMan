...
namespace RdcMan {
	public abstract class DisplaySettingsTabPage<TSettingsGroup> : SettingsTabPage<TSettingsGroup> where TSettingsGroup : CommonDisplaySettings {
		...
		protected void Create(out int rowIndex, out int tabIndex) {
			...
			FormTools.AddLabeledEnumDropDown(this, "Í£¿¿ RD Ëõ·Å(&D)", base.Settings.SmartSizeDockedWindow, ref rowIndex, ref tabIndex, RdpClient.SmartSizeMethodToString);
			FormTools.AddLabeledEnumDropDown(this, "È¡ÏûÍ£¿¿ RD Ëõ·Å(&U)", base.Settings.SmartSizeUndockedWindow, ref rowIndex, ref tabIndex, RdpClient.SmartSizeMethodToString);
		}
	}
}
