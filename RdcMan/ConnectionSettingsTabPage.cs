...
namespace RdcMan
{
	public class ConnectionSettingsTabPage : SettingsTabPage<ConnectionSettings>
	{
			...
			: base(dialog, settings)
		{
			...
			FormTools.AddCheckBox(this, "连接到控制台(&C)", settings.ConnectToConsole, 1, ref rowIndex, ref tabIndex);
			FormTools.AddLabeledTextBox(this, "启动程序(&S)：", settings.StartProgram, ref rowIndex, ref tabIndex);
			FormTools.AddLabeledTextBox(this, "工作目录(&W)：", settings.WorkingDir, ref rowIndex, ref tabIndex);
			...
			FormTools.AddLabeledTextBox(this, "负载均衡配置(&L)：", settings.LoadBalanceInfo, ref rowIndex, ref tabIndex);
