...
namespace RdcMan {
	public class FileGroup : GroupBase {
		...
		public sealed override bool ConfirmRemove(bool askUser) {
			FormTools.InformationDialog("使用文件菜单关闭 " + base.Text + " 组");
			return false;
		}
		...
		internal void CheckCredentials() {
			...
			this.VisitNodes(delegate (RdcTreeNode node) {
				CheckCredentials(node, node.LogonCredentials, "登录凭证", missingProfiles);
				CheckCredentials(node, node.GatewaySettings, "网关设置", missingProfiles);
			});
			if (missingProfiles.Count <= 0) {
				return;
			}
			StringBuilder stringBuilder = new StringBuilder();
			stringBuilder.AppendLine("未找到某些凭证配置文件。")
				.AppendLine("请在连接到服务器、编辑属性等之前添加缺少的配置文件，否则参考将丢失。")
				.AppendLine("单击 确定 将详细信息复制到剪贴板。")
				.AppendLine();
