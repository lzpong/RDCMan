	...
	public abstract class GroupBase : RdcTreeNode
	{
		...
		internal bool HasServers
		{
			get
			{
				...
				return base.Nodes[0] is ServerBase;
			}
		}
		...
		public bool HasGroups
		{
			get
			{
				if (base.Nodes.Count > 0)
				{
					return base.Nodes[0] is GroupBase;
				}
				return false;
			}
		}
		...
		static GroupBase()
		{
			...
			PropertyActions = new Dictionary<string, Helpers.ReadXmlDelegate>
			{
				...
				},
				{
					...
					delegate(XmlNode childNode, RdcTreeNode node, ICollection<string> errors)
					{
						bool.TryParse(childNode.InnerText, out bool result);
		...
		public override void ConnectAs(LogonCredentials logonSettings, ConnectionSettings connectionSettings)
		{
			...
			{
				DialogResult dialogResult = FormTools.YesNoDialog(base.Text + " 组中包含 " + count + " 个未连接的服务器. 你确定吗?");
		...
		public override bool ConfirmRemove(bool askUser)
		{
			...
			AnyOrAllConnected(out bool anyConnected, out bool _);
			if (anyConnected)
			{
				FormTools.InformationDialog(base.Text + " 组中有活动会话. 删除群组之前先断开它们的连接.");
				return false;
			}
			if (askUser && base.Nodes.Count > 0)
			{
				DialogResult dialogResult = FormTools.YesNoDialog("删除群组 " + base.Text + " ?");
				if (dialogResult != DialogResult.Yes)
				{
					return false;
				}
