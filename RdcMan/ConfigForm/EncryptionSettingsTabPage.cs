	...
	public class EncryptionSettingsTabPage : SettingsTabPage<EncryptionSettings>
	{
		...
		protected void SetSelectedCertificate(X509Certificate2 cert)
		{
			if (cert != null)
			{
				...
				_passwordEncryptionDataInfoLabel.Text = "有效期从 {0} 到 {1}".InvariantFormat(cert.NotBefore.ToUniversalTime().ToShortDateString(), cert.NotAfter.ToUniversalTime().ToShortDateString());
			}
		}
