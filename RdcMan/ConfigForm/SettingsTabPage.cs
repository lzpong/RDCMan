...
namespace RdcMan
{
	...
	public abstract class SettingsTabPage<TSettingsGroup> : SettingsTabPage, ISettingsTabPage where TSettingsGroup : SettingsGroup
	{
		...
		{
			TSettingsGroup settings = Settings;
			if (settings.InheritSettingsType.Mode != InheritanceMode.Disabled)
			{
				TabbedSettingsDialog dialog = Dialog;
				TSettingsGroup settings2 = Settings;
				InheritanceControl = new InheritanceControl(dialog, settings2.Name);
		...
		protected override void UpdateControls()
		{
			...
			{
				InheritanceControl inheritanceControl = InheritanceControl;
				TSettingsGroup settings = Settings;
